...
public class OrmController {
	...
	public @ResponseBody Message ORMController(@RequestBody GenericMessage genericMessage) {
		Create create = new Create();
		if (genericMessage.getEvn().equals("O01")) {
			Message response = new Message();
			String data;
			try {
				data = create.CreateADT_ORM01(genericMessage.getMshModel().getSendingApplication(), genericMessage.getPidModel().getSurName(), genericMessage.getPidModel().getName(), 
